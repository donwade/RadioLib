...
##derived from above. Modified specifically for intercepting oregon scientific sensors.
   requires one TTGO OLED board with 433Mhz radio (SX1768)
   Don't use this for any other purpose.

   Uses threads, mutexes, semaphores, isrSignals so its not really dumb-rookie arduino project

   Logs to an SD card OR can be controlled by a JTAG Jlink EDU device 

   Special thanks to others who worked on oregon before.
   (using modifed crc/csum routines from url=https://github.com/invandy/Oregon_NR
